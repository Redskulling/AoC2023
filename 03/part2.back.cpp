...
std::array data = std::to_array<std::string>({
...
template <size_t size>
size_t connectedNumber(const std::array<std::string, size> &data, size_t x, size_t y, size_t startx, size_t starty) {
	s32 ymax = 1;
	s32 xmax = 1;
	s32 xmin = startx - x;
	s32 ymin = starty - y;
...
size_t findConnected(const std::array<std::string, size> &data, size_t x, size_t y) {
	...
	std::string number = getNumber(data, x, y);

	for (size_t xpos = x; xpos < x + number.size(); xpos++) {
		size_t symx = xpos;
		size_t symy = y;
		if (connectedToSymbol(data, symx, symy)) {
			if (countAroundSymbol(data, symx, symy) != 2)
				continue;
			size_t num = connectedNumber(data, symx, symy, xpos + number.size(), y);
			if (num) {
				// std::cout << number << "*" << num << " = ";
				return std::stoi(number) * num;
			}
		}
	}
	return 0;
